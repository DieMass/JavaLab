...
public class CustomSetupTest {
	...
	@BeforeEach
	public void setUp() {
		when(customSetupService.update(1L)).thenReturn(publishedSetup());
	}
	...
	public void coursePublishTest() throws Exception {
		mockMvc.perform(put("/customSetups/1/publish")).andDo(print())
				...
				.andExpect(jsonPath("$.name").value(publishedSetup().getName()))
				.andExpect(jsonPath("$.status").value(publishedSetup().getStatus().name()))
	...
	private CustomSetup publishedSetup() {
		return CustomSetup.builder().id(1L).name("best").status(Status.PUBLISH).build();
	}
}
