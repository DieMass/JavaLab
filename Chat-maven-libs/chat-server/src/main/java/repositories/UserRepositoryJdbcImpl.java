...
public class UserRepositoryJdbcImpl implements UserRepository {
    ...
    private RowMapper<die.mass.models.User> userRowMapper = row -> {
        ...
        Long password = row.getLong("password");
        return new die.mass.models.User(id, name, password);
    };
    //аналог insert в SQL
    @Override
    public void save(die.mass.models.User model) {
        try {
            PreparedStatement statement = connection.prepareStatement(SQL_INSERT_USER,
                    Statement.RETURN_GENERATED_KEYS);
            statement.setString(1, model.getName());
            statement.setLong(2, model.getPassword());
    ...
    @Override
    public boolean contains(String name, Long password) {
        die.mass.models.User user = null;
        try {
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery("select * from account where name = '" + name + "' and password = " + password + ";");
